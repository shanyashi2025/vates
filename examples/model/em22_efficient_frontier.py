...
def main(workspace_directory: str, input_directories: list[str]):
    ...
    df = model_space.read_csv('parameters', index_col='parameter')
    risk_free_rate = df.loc['risk_free_rate', 'value']
    ret_tar = df.loc['return_target', 'value']
    std_tol = df.loc['standard_deviation_tolerance', 'value']
    df = model_space.read_csv('assets', index_col="asset_name")
    ...
    for i in range(n_assets):
        ...
        mu[i] = df.loc[asset_name, 'expected_return']
    ...
    output_folder, output_file_name = output_config_df.loc['portfolios_csv', ['output_folder', 'output_file_name']]
