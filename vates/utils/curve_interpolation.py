...
def curve_linear_interp(x: npt.NDArray[np.float64], y: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
    """
...
def curve_loglinear_interp(x: npt.NDArray[np.float64], y: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
    """
...
def curve_geometric_interp(x: npt.NDArray[np.float64], y: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
    """
...
def curve_next_interp(x: npt.NDArray[np.float64], y: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
    """
...
def zcb_exponential_interp(x: npt.NDArray[np.float64], y: npt.NDArray[np.float64], time_interval: float,
                           forward_rate_cc: float) -> npt.NDArray[np.float64]:
    """
...
def curve_interp(x: npt.NDArray[np.float64], y: npt.NDArray[np.float64], kind: str, **kwargs) -> npt.NDArray[np.float64]:
    """
    ...
    Args:
        x: Array of x-coordinates (must be sorted in ascending order)
        y: Array of y-coordinates corresponding to x
        kind: Interpolation kind to use
    ...
    if kind == "linear":
        return curve_linear_interp(x, y)
    elif kind in ("loglinear", "log_linear"):
        return curve_loglinear_interp(x, y)
    elif kind == "geometric":
        return curve_geometric_interp(x, y)
    elif kind in ("next", "next_point"):
        return curve_next_interp(x, y)
    elif kind == 'zcb_exponential':
        return zcb_exponential_interp(x, y, kwargs['time_interval'], kwargs.get('forward_rate_cc', math.log(1.045)))
    else:
